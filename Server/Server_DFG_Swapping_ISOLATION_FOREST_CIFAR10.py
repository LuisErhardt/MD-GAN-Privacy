...
def swap_decision_single_isof(user1_id, user2_id, result_vector):
    '''
    ...
    clf = IsolationForest(random_state=0).fit(result_vector.reshape(-1,1))
    isof = clf.predict(result_vector.reshape(-1,1))
    ...
    cluster_negative = cluster_map[cluster_map.cluster == -1]
    cluster_positive = cluster_map[cluster_map.cluster == 1]
    if cluster_positive['data'].mean() >= cluster_negative['data'].mean():
        print("in general, this should not happen, because benign should have lower distance, cluster_positive['data'].mean() should be smaller than other group!")
        if user2_id in cluster_negative.index:
            # return True only if user2_id is in the group where their average distance to user1_id is smaller
            return True
        else:
            return False
    else:
        if user2_id in cluster_positive.index:
            # return True only if user2_id is in the group where their average distance to user1_id is smaller
            return True
        else:
            return False
...
def swap_decision(user1_id, user2_id, result_matrix):
    ...
    decision1 = swap_decision_single_isof(user1_id, user2_id, result_matrix[user1_id])
    decision2 = swap_decision_single_isof(user2_id, user1_id, result_matrix[user2_id])
...
def ignore_clients_chosen_isof(prediction_result_list, local_fr_discriminator_index, local_benign_discriminator_index):
    cluster_map = pd.DataFrame()

    # isolation forest
    clf = IsolationForest(random_state=0).fit(prediction_result_list)
    isof = clf.predict(prediction_result_list)
    print("isof prediction: ", isof)
    cluster_map['isof'] = isof
    cluster_negative = cluster_map[cluster_map.isof == -1]
    cluster_positive = cluster_map[cluster_map.isof == 1]
    k_biggest_indices_isof = []
    isof_flag = 1
    if local_benign_discriminator_index in cluster_positive.index:
        k_biggest_indices_isof = list(cluster_negative.index)
    else:
        # that means local_benign should in cluster_positive, if it is not, that means the classifier works wrong
        # we skip the results from this classifier for this time
        print("isof_flag == 0")
        isof_flag = 0
    if isof_flag == 1 and local_fr_discriminator_index in k_biggest_indices_isof:
        k_biggest_indices_isof.remove(local_fr_discriminator_index)
    print("isof decision: ", k_biggest_indices_isof)


    final_ignore_list = k_biggest_indices_isof

    if len(final_ignore_list) == (len(prediction_result_list)-2):
        final_ignore_list = []
    print("final_ignore_list ", final_ignore_list)
            ...
            for i in range(self.steps_per_epoch):
                ...
                if i % self.n_critic == 0:
                    ...
                    # if j % 1 == 0 :
                        ...
                        self.ignore_clients = ignore_clients_chosen_isof(prediction_result_list, local_fr_discriminator_index, local_benign_discriminator_index)
                        self.ignore_clients_record.append(self.ignore_clients)
