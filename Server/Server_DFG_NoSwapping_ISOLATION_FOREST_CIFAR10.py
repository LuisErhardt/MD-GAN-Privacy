...
def ignore_clients_chosen_isof(prediction_result_list, local_fr_discriminator_index, local_benign_discriminator_index):
    ...
    # in the current setting, where the benign discriminat sets in, it's the benign group
    # we don't care the label is 1 or -1 from isolation forest
    if local_benign_discriminator_index in cluster_positive.index:
        k_biggest_indices_isof = list(cluster_negative.index)
    else:
        k_biggest_indices_isof = list(cluster_positive.index)

    if local_fr_discriminator_index in k_biggest_indices_isof:
        k_biggest_indices_isof.remove(local_fr_discriminator_index)

    ## following block is original setting, but does not work well    
    # else:
